...
class Captcha:
    ...
    CAPTCHA_USER_AGENT = "28,1023,androidPhoneApp,1600x900,1.5,Redmi,23113RKC6C,7f2821773c5c1659,1783364599220"
    ...
    def solve_captcha(self):
        ...
        proxy_parts = self.parse_proxy_url(proxy_url)
        ...
        payload = {
            ...
            "task": {
                ...
                "websiteURL": self.CAPTCHA_PAGE_URL,
                "websiteKey": self.CAPTCHA_SITE_KEY,
                "userAgent": self.CAPTCHA_USER_AGENT,
                **proxy_parts
            }
        }

        response = requests.post(create_task_url, json=payload, proxies=proxies)
        ...
        while True:
            time.sleep(5)
            res = requests.post(get_result_url, json=result_payload, proxies=proxies).json()
